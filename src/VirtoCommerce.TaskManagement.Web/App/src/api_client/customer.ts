...
export class CustomerModuleClient extends AuthApiBase {
    ...
    resizeIcon(body?: IconResizeRequest | null | undefined): Promise<void> {
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processResizeIcon(_response);
        });
    }

    protected processResizeIcon(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 204) {
            return response.text().then((_responseText) => {
...
export class IconResizeRequest implements IIconResizeRequest {
    url?: string | undefined;
    width?: number | undefined;
    height?: number | undefined;

    constructor(data?: IIconResizeRequest) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): IconResizeRequest {
        data = typeof data === 'object' ? data : {};
        let result = new IconResizeRequest();
        result.init(data);
        return result;
    }
...
export interface IIconResizeRequest {
    url?: string | undefined;
    width?: number | undefined;
    height?: number | undefined;
}
